...
if (isset($_GET['bulk_action'])) {
	...
	if ($ids) {
		...
		if ($_GET['bulk_action'] == 'delete') {
			...
			$stmt = $pdo->prepare('DELETE FROM contacts WHERE id IN (' . implode(',', array_fill(0, count($ids), '?')) . ')');
			$stmt->execute($ids);
		}
...
$stmt = $pdo->prepare('SELECT * FROM contacts ' . $where_sql . ' ORDER BY ' . $order_by . ' ' . $order_sort . $limit_sql);
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM contacts ' . $where_sql);
// Bind the search query param to the SQL query
if (isset($_GET['search']) && !empty($_GET['search'])) {	
	$stmt->bindValue(':search_query', '%' . $_GET['search'] . '%');
}
				...
				<thead>
					<tr>
						...
						<td<?=$order_by=='id'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=id&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								#
								<?php if ($order_by == 'id'): ?>
						...
						<td<?=$order_by=='first_name'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=first_name&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								First Name
						...
						<td<?=$order_by=='last_name'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=last_name&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								Last Name
						...
						<td<?=$order_by=='email'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=email&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								Email
						...
						<td<?=$order_by=='phone'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=phone&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								Phone
						...
						<td<?=$order_by=='title'?' class="active"':''?>>
							<a href="read.php?page=1&records_per_page=<?=$records_per_page?>&order_by=title&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
								Title
