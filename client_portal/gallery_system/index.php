...
$stmt = $pdo->prepare('SELECT * FROM collections WHERE is_public = 1 ORDER BY title');
$stmt->execute();
...
$collection = isset($_GET['collection']) && is_numeric($_GET['collection']) ? (int)$_GET['collection'] : 'all';
...
$sort_by = isset($_GET['sort_by']) && in_array($_GET['sort_by'], ['newest', 'oldest', 'a_to_z', 'z_to_a']) ? $_GET['sort_by'] : 'newest';
...
$type = isset($_GET['type']) && in_array($_GET['type'], ['image', 'video', 'audio']) ? $_GET['type'] : 'all';
$type_sql = $type != 'all' ? 'AND m.media_type = :type' : '';
// Handle search query
...
$stmt = $pdo->prepare('SELECT m.*, a.display_name, (SELECT COUNT(*) FROM media_likes ml WHERE ml.media_id = m.id) AS likes, (SELECT COUNT(*) FROM media_likes ml WHERE ml.media_id = m.id AND ml.acc_id = :acc_id) AS liked FROM media m ' . $collection_sql . ' LEFT JOIN accounts a ON a.id = m.acc_id WHERE m.is_approved = 1 AND m.is_public = 1 ' . $type_sql . ' ' . $search_sql . ' ORDER BY ' . $sort_by_sql . ' LIMIT :page,:media_per_page');
// Bind the account ID
...
?>
<?=template_header(isset($_GET['search']) && $_GET['search'] ? 'Search Results for "' . htmlspecialchars($_GET['search'], ENT_QUOTES) . '"' : 'Gallery')?>

<div class="page-top">
	<div class="container">
		<?php if (isset($_GET['search']) && $_GET['search']): ?>
		<h2 class="title">Search Results for "<?=htmlspecialchars($_GET['search'], ENT_QUOTES)?>"</h2>
		<?php elseif (get_collection_by_id($collections, $collection)): ?>
		<h2 class="title"><?=htmlspecialchars(get_collection_by_id($collections, $collection)['title'], ENT_QUOTES)?></h2>
		<p class="description"><?=htmlspecialchars(get_collection_by_id($collections, $collection)['description_text'], ENT_QUOTES)?></p>
		<?php else: ?>
		<h2 class="title">Gallery</h2>
		<?php endif; ?>
		<div class="navigation-links">
			<?php $nav_base_params = ['sort_by' => $sort_by, 'collection' => $collection, 'search' => isset($_GET['search']) ? $_GET['search'] : '']; ?>
...
<div class="page-content">
	...
	<div class="media-list-filters">
		...
		<form class="media-list-filters-form" action="index.php" method="get">
			...
			<label for="search">
				...
				<input id="search" type="text" name="search" placeholder="Search..." value="<?=isset($_GET['search']) ? htmlspecialchars($_GET['search'], ENT_QUOTES) : ''?>">
			</label>
		</form>
	</div>
	...
	<div class="media-list layout-<?=media_layout?>">
		...
		<div class="masonry-column" data-column-index="<?=$col_index?>">
			<?php foreach ($column as $i => $m): ?>
			<a href="<?=media_popup ? '#' : 'view.php?id=' . $m['id']?>" data-src="<?=$m['filepath']?>" data-id="<?=$m['id']?>" data-index="<?=$m['index']?>" data-autoplay="<?=media_autoplay?>" data-user-collections="<?=htmlspecialchars($user_collections, ENT_QUOTES)?>" data-title="<?=htmlspecialchars($m['title'], ENT_QUOTES)?>" data-description="<?=htmlspecialchars($m['description_text'], ENT_QUOTES)?>" data-type="<?=$m['media_type']?>" data-likes="<?=$m['likes']?>" data-liked="<?=$m['liked']?1:0?>" data-uploaded-date="<?=date('F j, Y', strtotime($m['uploaded_date']))?>" data-user="<?=$m['display_name'] ? htmlspecialchars($m['display_name'], ENT_QUOTES) : 'Anonymous'?>" data-original-column="<?=$col_index?>"<?=isset($_SESSION['account_loggedin']) && $_SESSION['account_id'] == $m['acc_id']?' data-own-media':''?>>
				<?php if (file_exists($m['filepath']) && $m['media_type'] == 'image' && !file_exists($m['thumbnail'])): ?>
				<img src="<?=$m['filepath']?>" alt="<?=htmlspecialchars($m['title'], ENT_QUOTES)?>" width="<?=getimagesize($m['filepath'])[0]?>" height="<?=getimagesize($m['filepath'])[1]?>">
				<?php elseif ($m['media_type'] == 'video' && empty($m['thumbnail'])): ?>
				...
				<?php elseif ($m['media_type'] == 'audio' && empty($m['thumbnail'])): ?>
				<span class="placeholder">
				...
				<img src="<?=$m['thumbnail']?>" alt="<?=htmlspecialchars($m['title'], ENT_QUOTES)?>" width="<?=getimagesize($m['thumbnail'])[0]?>" height="<?=getimagesize($m['thumbnail'])[1]?>">
				<?php else: ?>
				<span class="placeholder">
		...
		<a href="<?=media_popup ? '#' : 'view.php?id=' . $m['id']?>" data-src="<?=$m['filepath']?>" data-id="<?=$m['id']?>" data-index="<?=$i?>" data-autoplay="<?=media_autoplay?>" data-user-collections="<?=htmlspecialchars($user_collections, ENT_QUOTES)?>" data-title="<?=htmlspecialchars($m['title'], ENT_QUOTES)?>" data-description="<?=htmlspecialchars($m['description_text'], ENT_QUOTES)?>" data-type="<?=$m['media_type']?>" data-likes="<?=$m['likes']?>" data-liked="<?=$m['liked']?1:0?>" data-uploaded-date="<?=date('F j, Y', strtotime($m['uploaded_date']))?>" data-user="<?=$m['display_name'] ? htmlspecialchars($m['display_name'], ENT_QUOTES) : 'Anonymous'?>"<?=isset($_SESSION['account_loggedin']) && $_SESSION['account_id'] == $m['acc_id']?' data-own-media':''?>>
			<?php if (file_exists($m['filepath']) && $m['media_type'] == 'image' && !file_exists($m['thumbnail'])): ?>
			<img src="<?=$m['filepath']?>" alt="<?=htmlspecialchars($m['title'], ENT_QUOTES)?>" width="<?=getimagesize($m['filepath'])[0]?>" height="<?=getimagesize($m['filepath'])[1]?>">
			<?php elseif ($m['media_type'] == 'video' && empty($m['thumbnail'])): ?>
			<span class="placeholder">
	...
	<div class="pagination">
        <?php
        $query_params = ['sort_by' => $sort_by, 'collection' => $collection, 'type' => $type, 'search' => isset($_GET['search']) ? $_GET['search'] : ''];
