...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_GET['method'], $_POST['email'], $_POST['password']) && $_GET['method'] == 'login') {
        ...
        if ($account) {
            ...
            if (password_verify($_POST['password'], $account['password'])) {
                $_SESSION['review_account_loggedin'] = TRUE;
                $_SESSION['review_account_id'] = $account['id'];
                $_SESSION['review_account_display_name'] = $account['display_name'];
                $_SESSION['review_account_role'] = $account['role']; 
                $_SESSION['review_account_email'] = $account['email'];
                exit('success');
            } else {
    ...
    if (isset($_GET['method'], $_POST['email'], $_POST['password'], $_POST['cpassword'], $_POST['name']) && $_GET['method'] == 'register') {
        ...
        // Authenticate user
        $_SESSION['review_account_loggedin'] = TRUE;
        $_SESSION['review_account_id'] = $pdo->lastInsertId();
        $_SESSION['review_account_display_name'] = $_POST['name'];
        $_SESSION['review_account_role'] = 'Member';
        $_SESSION['review_account_email'] = $_POST['email'];
        exit('success');
    }
    // Get type
    ...
    if ($type == 'full' && isset($_POST['rating'], $_POST['content'])) {
        ...
        if (authentication_required && !isset($_SESSION['review_account_loggedin'])) {
            exit('Error: Please login to post a review!');    
        }
        // Declare review variables
        $acc_id = isset($_SESSION['review_account_loggedin']) ? $_SESSION['review_account_id'] : -1; 
        $name = isset($_SESSION['review_account_loggedin']) ? $_SESSION['review_account_display_name'] : $_POST['name']; 
...
<div class="overall-rating" itemscope itemprop="aggregateRating" itemtype="https://schema.org/AggregateRating">
    ...
    <span class="stars" title="<?=number_format($reviews_info['overall_rating'], 1)?> star">
        <?=str_repeat('<i class="fa-solid fa-star star"></i>', round($reviews_info['overall_rating']))?>
        <?php if (max_stars-round($reviews_info['overall_rating']) > 0): ?>
        <?=str_repeat('<i class="fa-solid fa-star star-alt"></i>', max_stars-round($reviews_info['overall_rating']))?>
        <?php endif; ?>
    </span>
    <span class="total"><span itemprop="ratingCount"><?=$reviews_info['total_reviews']?></span> reviews</span>
    <?php if ($type == 'full'): ?>
    <a href="#" class="toggle-breakdown-button"><i class="fa-solid fa-<?=$breakdown_status=='open'?'minus':'plus'?>"></i></a>
    <?php endif; ?>
</div>
...
<div class="review-header">
    <div class="review-btns">
        <?php if (authentication_required && !isset($_SESSION['review_account_loggedin'])): ?>
        <a href="#" class="login-btn"><i class="fa-solid fa-lock fa-sm"></i>Login</a>
        <a href="#" class="register-btn"><i class="fa-solid fa-user fa-sm"></i>Register</a>
        <?php else: ?>
        <a href="#" class="write-review-btn">Write Review</a>
        <?php endif; ?>
    </div>
    <div class="sort-by">
        <a href="#">Sort by <?=isset($_GET['sort_by']) && empty($where) ? htmlspecialchars(ucwords(str_replace('_', ' ', $_GET['sort_by'])), ENT_QUOTES) : 'Newest'?><i class="fa-solid fa-caret-down fa-sm"></i></a>
        <div class="options">
...
<div class="write-review">
    <form autocomplete="off">
        <?php if (!isset($_SESSION['review_account_loggedin'])): ?>
        <input name="name" type="text" placeholder="Your Name" required>
        <?php endif; ?>
        <input name="rating" class="rating" type="number">
        <div class="stars">
            <?php for($i = 1; $i <= max_stars; $i++): ?>
            <i class="fa-solid fa-star star" data-id="<?=$i?>"></i>
            <?php endfor; ?>
        </div>
        <div class="content">
    ...
    <div>
        ...
        <div class="con">
            <span class="rating" itemprop="reviewRating" itemscope itemtype="https://schema.org/Rating" title="<?=$review['rating']?> star">
                <?=str_repeat('<i class="fa-solid fa-star star"></i>', $review['rating'])?>
                <?php if (max_stars-$review['rating'] > 0): ?>
                <?=str_repeat('<i class="fa-solid fa-star star-alt"></i>', max_stars-$review['rating'])?>
                <?php endif; ?>
        ...
        <?php endif; ?>
        <a href="#" class="like-btn" data-id="<?=$review['id']?>"><i class="fa-solid fa-thumbs-up"></i>Like</a>
        <?php if (isset($_SESSION['review_account_loggedin']) && $_SESSION['review_account_role'] == 'Admin'): ?>
        <a target="_blank" href="<?=reviews_directory_url?>admin/review.php?id=<?=$review['id']?>" class="edit-btn"><i class="fa-solid fa-pen fa-sm"></i>Edit</a>
        <?php endif; ?>
