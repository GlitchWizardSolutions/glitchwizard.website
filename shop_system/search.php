...
if (isset($_GET['query']) && $_GET['query'] != '') {
    ...
    $stmt = $pdo->prepare('SELECT p.*, (SELECT m.full_path FROM shop_product_media_map pm JOIN shop_product_media m ON m.id = pm.media_id WHERE pm.product_id = p.id ORDER BY pm.position ASC LIMIT 1) AS img FROM shop_products p WHERE p.product_status = 1 AND p.title LIKE ? ORDER BY p.created DESC');
