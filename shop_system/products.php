...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM (SELECT p.id FROM shop_products p ' . $category_sql . ' ' . $options_sql . ' WHERE p.product_status = 1  ' . $price_sql . ' ' . $availability_sql . ' GROUP BY p.id) q');
if ($category_list) {
    $stmt->bindValue(':category_list', implode(',', $category_list), PDO::PARAM_STR);
}
if ($options_list) {
...
<div class="products content-wrapper">
    ...
    <form action="<?=url('products.php')?>" method="get" class="products-form form">
        ...
        <div class="products-view">
            ...
            <div class="products-wrapper">
                ...
                <a href="<?=url('product.php?id=' . ($product['url_slug'] ? $product['url_slug']  : $product['id']))?>" class="product<?=$product['quantity']==0?' no-stock':''?>">
