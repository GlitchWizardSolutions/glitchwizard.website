...
if (!isset($_GET['id']) || !$logged_in || !$rowusers) {
    exit('Invalid request!');
}
// Prepare and execute the SQL query to fetch the product download
$stmt = $pdo->prepare('SELECT pd.* FROM shop_product_downloads pd JOIN shop_transactions t ON t.account_id = ? JOIN shop_transaction_items ti ON t.txn_id = ti.txn_id AND ti.item_id = pd.product_id AND MD5(CONCAT(ti.txn_id, pd.id)) = ?');
$stmt->execute([ $rowusers['id'], $_GET['id'] ]);
