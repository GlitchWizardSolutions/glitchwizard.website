<?php
/*
ENHANCED CHECKOUT PROCESSING - Step 4 Security Improvements
Added: CSRF protection, enhanced validation, better error handling
*/
...
if (!isset($_SESSION['checkout_csrf_token'])) {
...
if (!isset($_SESSION['checkout_attempts'])) {
    ...
    $_SESSION['checkout_last_attempt'] = time();
}

// Check for too many attempts
if ($_SESSION['checkout_attempts'] > 5 && (time() - $_SESSION['checkout_last_attempt']) < 300) {
...
// Enhanced security: Validate cart contents
$valid_cart = true;
...
if ($_POST && !empty($errors) == false) {
    // CSRF token validation
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['checkout_csrf_token'], $_POST['csrf_token'])) {
        $errors[] = 'Invalid security token. Please refresh the page and try again.';
    }
    
    // Update checkout attempts
    $_SESSION['checkout_attempts']++;
    $_SESSION['checkout_last_attempt'] = time();
}

// Update discount code with enhanced validation
if (isset($_POST['discount_code'])) {
    ...
    } elseif (strlen($discount_code) > 50) {
        $errors[] = 'Discount code is too long.';
    } elseif (!preg_match('/^[A-Za-z0-9\-_]+$/', $discount_code)) {
        $errors[] = 'Invalid discount code format.';
    } else {
        $_SESSION['discount'] = $discount_code;
    }
} elseif (isset($_POST['discount_code'], $_SESSION['discount']) && empty($_POST['discount_code'])) {
    unset($_SESSION['discount']);
}
// Variables
...
$stmt = $pdo->prepare('SELECT p.*, (SELECT m.full_path FROM shop_product_media_map pm JOIN shop_product_media m ON m.id = pm.media_id WHERE pm.product_id = p.id ORDER BY pm.position ASC LIMIT 1) AS img, (SELECT GROUP_CONCAT(pc.category_id) FROM shop_product_category pc WHERE pc.product_id = p.id) AS categories FROM shop_products p WHERE p.id IN (' . $array_to_question_marks . ')');
...
$shipping_methods = $pdo->query('SELECT * FROM shop_shipping')->fetchAll(PDO::FETCH_ASSOC);
// Retrieve the discount code
if (isset($_SESSION['discount'])) {
    $stmt = $pdo->prepare('SELECT * FROM shop_discounts WHERE discount_code = ?');
    $stmt->execute([ $_SESSION['discount'] ]);
    $discount = $stmt->fetch(PDO::FETCH_ASSOC);
}
// Get tax
$stmt = $pdo->prepare('SELECT * FROM shop_taxes WHERE country = ?');
...
if (isset($_POST['method'], $_POST['first_name'], $_POST['last_name'], $_POST['address_street'], $_POST['address_city'], $_POST['address_state'], $_POST['address_zip'], $_POST['address_country']) && !isset($_POST['update'])) {
    // Account ID
    $account_id = null;
    // If the user is already logged in, update the user's details
    if ($logged_in && $rowusers) {
        ...
        $stmt->execute([ $_POST['first_name'], $_POST['last_name'], $_POST['address_street'], $_POST['address_city'], $_POST['address_state'], $_POST['address_zip'], $_POST['address_country'], $account['id'] ]);
    ...
    if (!$errors) {
        ...
        // Log the user in with the details provided (this section is no longer needed since we use main site auth)
        // Note: User authentication is handled by the main site auth system, not here
        ...
        if (pay_on_delivery_enabled && $_POST['method'] == 'payondelivery') {
            ...
            $stmt = $pdo->prepare('INSERT INTO shop_transactions (txn_id, payment_amount, payment_status, created, payer_email, first_name, last_name, address_street, address_city, address_state, address_zip, address_country, account_id, payment_method, shipping_method, shipping_amount, discount_code, tax_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
            ...
            header('Location: ' . url('placeorder.php'));
            exit;
        }
    }
...
<div class="checkout content-wrapper">
    ...
    <?php if (!$logged_in): ?>
    <p>Already have an account? <a href="../auth.php?redirect=<?=urlencode($_SERVER['REQUEST_URI'])?>" class="link">Log In</a></p>
    <?php endif; ?>

    <form action="checkout.php" method="post" class="form pad-top-2">
        <!-- Enhanced Security: CSRF Protection -->
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['checkout_csrf_token']) ?>">
        
        <!-- Enhanced Security: Form Validation Timestamp -->
