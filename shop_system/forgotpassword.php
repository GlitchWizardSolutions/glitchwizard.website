    ...
    if ($account) {
        ...
        if (isset($_POST['npassword'], $_POST['cpassword'])) {
            if (strlen($_POST['npassword']) > 20 || strlen($_POST['npassword']) < 5) {
				...
				// Output success message
                $success_msg = 'Password has been reset! You can now <a href="' . url('myaccount.php') . '" class="form-link">login</a>!';
            }
        }
    } else {
...
<div class="content-wrapper forgot-password">
    ...
    <?php if ($page == 'Forgot'): ?>
    <form action="<?=url('forgotpassword.php')?>" method="post" class="form">
    ...
    <?php else: ?>
    <form action="<?=url('forgotpassword.php', ['code' => htmlspecialchars($_GET['code'], ENT_QUOTES)])?>" method="post" class="form">
