...
class ContactForm {
    ...
    processForm(formData) {
        ...
        // Prefer BrandSpinner helper if globally available; fallback to Bootstrap Icon spinner
        if (window.BrandSpinner && BrandSpinner.buttonLoading) {
            // Store original for manual fallback revert just in case
            this.container.querySelector('[type="submit"]').dataset.originalHtml = btnTxt;
            try { BrandSpinner.buttonLoading(this.container.querySelector('[type="submit"]'), {label:'', size:'sm'}); } catch(e) {}
        } else {
            this.container.querySelector('[type="submit"]').innerHTML = '<span class="spinner-border spinner-border-sm align-text-bottom" role="status" aria-hidden="true"></span>';
        }
                ...
                for (let i = 0; i < event.target.files.length; i++) {
                    ...
                    if (file.type.includes('image')) {
                        ...
                        reader.onload = readerEvent => {
                            ...
                            element.insertAdjacentHTML('beforebegin', `<div class="file" title="${file.name}" data-index="${arrIndex}"><img src="${content}" width="70" height="70"><a href="#" class="remove-file" aria-label="Remove file"><i class="bi bi-x-circle-fill" aria-hidden="true"></i></a></div>`);
                        ...
                        element.insertAdjacentHTML('beforebegin', `<div class="file" title="${file.name}" data-index="${arrIndex}"><i class="bi bi-file-earmark-text" aria-hidden="true"></i><a href="#" class="remove-file" aria-label="Remove file"><i class="bi bi-x-circle-fill" aria-hidden="true"></i></a></div>`);
                        // Add onclick event to remove the file
                        element.parentElement.querySelectorAll('.remove-file').forEach(el => el.onclick = event => {
                            event.preventDefault();
                            // Remove the file from the uploadFiles array
