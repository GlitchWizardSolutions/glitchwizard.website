...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shop_shipping SET title = ?, price_from = ?, price_to = ?, weight_from = ?, weight_to = ?, price = ?, shipping_type = ?, countries = ? WHERE id = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM shop_shipping WHERE id = ?');
    ...
    if (isset($_POST['submit'])) {
        $countries_list = isset($_POST['countries']) ? implode(',', $_POST['countries']) : '';
        $stmt = $pdo->prepare('INSERT INTO shop_shipping (title, price_from, price_to, weight_from, weight_to, price, shipping_type, countries) VALUES (?,?,?,?,?,?,?,?)');
