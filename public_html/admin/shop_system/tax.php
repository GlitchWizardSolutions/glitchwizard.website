...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shop_taxes SET country = ?, rate = ?, rate_type = ?, rules = ? WHERE id = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM shop_taxes WHERE id = ?');
    ...
    if (isset($_POST['submit'])) {
        $rules = generate_rules();
        $stmt = $pdo->prepare('INSERT INTO shop_taxes (country,rate,rate_type,rules) VALUES (?,?,?,?)');
