...
?>
<?=template_admin_header('Invoice #' . $invoice['invoice_number'], 'invoices')?>
...
<div class="content-title responsive-flex-wrap responsive-pad-bot-3">
    ...
    <a href="invoices.php?delete=<?=$_GET['id']?>" class="btn red mar-right-2" onclick="return confirm('Are you sure you want to delete this invoice?')">Delete</a>
    <a href="invoice.php?id=<?=$_GET['id']?>" class="btn">Edit</a>
</div>
...
<div class="content-block-wrapper">
    <div class="content-block invoice-details">
        ...
        <div class="invoice-detail">
            <h3>Due Date</h3>
            <p><?=date('F j, Y H:ia', strtotime($invoice['due_date']))?></p>
        </div>
        <div class="invoice-detail">
            <h3>Created</h3>
            <p><?=date('F j, Y H:ia', strtotime($invoice['created']))?></p>
        </div>
        <?php if (file_exists(base_path . 'pdfs/' . $invoice['invoice_number'] . '.pdf')): ?>
        <div class="invoice-detail">
            <h3>Attachment</h3>
            <p><a href="<?=base_url?>pdfs/<?=$invoice['invoice_number']?>.pdf" target="_blank" class="link1"><?=$invoice['invoice_number']?>.pdf</a></p>
        </div>
        <?php endif; ?>
        <div class="invoice-detail">
            <h3>Share Link</h3>
            <p style="max-width:250px"><a href="<?=base_url?>invoice.php?id=<?=$invoice['invoice_number']?>" class="link1" target="_blank"><?=base_url?>invoice.php?id=<?=$invoice['invoice_number']?></a></p>
        </div>       
    </div>
...
<div class="content-block">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <small class="text-muted">Detailed breakdown of invoice line items</small>
        </div>
        <div class="card-body">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>Description</th>
                            <th>Qty</th>
                            <th class="responsive-hidden">Price</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="fw-medium"><?=htmlspecialchars($item['item_name'], ENT_QUOTES)?></td>
                            <td class="text-muted"><?=htmlspecialchars($item['item_description'], ENT_QUOTES)?></td>
                            <td><span class="badge bg-light text-dark"><?=$item['item_quantity']?></span></td>
                            <td class="responsive-hidden"><?=currency_code?><?=number_format($item['item_price'], 2)?></td>
                            <td class="text-end fw-bold"><?=currency_code?><?=number_format($item['item_price']*$item['item_quantity'], 2)?></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
                    <tfoot class="table-light">
                        <tr>
                            <td colspan="4" class="fw-medium">Subtotal</td>
                            <td class="text-end fw-bold"><?=currency_code?><?=number_format($invoice['payment_amount'], 2)?></td>
                        </tr>
                        <?php if ($invoice['tax_total'] > 0): ?>
                        <tr>
                            <td colspan="4" class="fw-medium">Tax (<?=$invoice['tax']?>)</td>
                            <td class="text-end fw-bold"><?=currency_code?><?=number_format($invoice['tax_total'], 2)?></td>
                        </tr>
                        <?php endif; ?>
                        <tr class="table-success">
                            <td colspan="4" class="fw-bold">Total</td>
                            <td class="text-end fw-bold fs-5"><?=currency_code?><?=number_format($invoice['payment_amount']+$invoice['tax_total'], 2)?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
