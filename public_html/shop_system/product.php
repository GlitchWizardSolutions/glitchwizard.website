...
if (isset($_GET['id'])) {
    ...
    $on_wishlist = false;
    // Check if the user is logged in using main site auth
    ...
    $meta = '
        <meta property="og:url" content="' . url('product.php?id=' . ($product['url_slug'] ? $product['url_slug']  : $product['id'])) . '">
    ...
    if (isset($_POST['add_to_wishlist'])) {
        ...
        if ($logged_in && $rowusers) {
            ...
            if ($wishlist_item) {
                ...
                header('Location: product.php?id=' . ($product['url_slug'] ? $product['url_slug']  : $product['id']));
                exit;
            }
        } else {
    ...
    // Check if the user clicked the remove FROM shop_wishlist button
        ...
        if ($logged_in && $rowusers) {
            ...
            $stmt->execute([ $product['id'], $rowusers['id'] ]);
            // Redirect to the same page to prevent form resubmission issues
            header('Location: ' . url('product.php?id=' . ($product['url_slug'] ? $product['url_slug']  : $product['id'])));
            exit;
        } else {
            header('Location: ' . url('myaccount.php'));
            exit;
        }
        ...
        if (!$validation_error) {
            ...
            if ($quantity > 0) {
                ...
                if (isset($_POST['paypal_subscribe'])) {
                    header('Location: ' . url('subscribe.php?method=paypal'));
                    exit;
                }
                // If the user clicked the stripe subscribe button
                if (isset($_POST['stripe_subscribe'])) {
                    header('Location: ' . url('subscribe.php?method=stripe'));
                    exit;
                }
...
<div class="product content-wrapper">
    ...
    <div class="product-wrapper">
        ...
        <form class="product-form form" action="<?=url('product.php?id=' . ($product['url_slug'] ? $product['url_slug']  : $product['id']))?>" method="post">
