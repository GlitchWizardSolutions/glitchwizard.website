    ...
    fetchFileList(directory = null, directoryToken = null, sortOrder = null, sortBy = null, callback = null) {
        ...
        fetch(this.path + url, { cache: 'no-store' }).then(response => response.json()).then(obj => {
            if (obj.status == 'success') {
                ...
                if (!obj.isInitialDirectory && !this.header.querySelector('.path .wrapper a[data-directory="' + this.directory + '"]')) {
                    ...
                    `);
                } else if (obj.isInitialDirectory) {
                    ...
                    <div class="content">
                        ...
                        <div class="body">
                            <form class="file-manager-upload-form" action="upload.php?directory=${self.directory}&token=${self.directoryToken}" method="post" enctype="multipart/form-data">
                                <label for="files"><i class="fa-solid fa-folder-open fa-2x"></i>Select file(s) or drag and drop ...</label>
            ...
            onOpen: function (event) {
                ...
                this.filesInput.onchange = () => {
                    for (let i = 0; i < this.filesInput.files.length; i++) {
                        let index = this.files.push(this.filesInput.files[i]);
                        this.uploadForm.querySelector('.files-list').innerHTML += '<span><i class="fa-solid fa-file"></i>' + this.filesInput.files[i].name + '<i class="fa-solid fa-xmark remove-file-btn" data-index="' + index + '"></i></span>';
                    }
                    this.uploadForm.querySelectorAll('.files-list .remove-file-btn').forEach(element => element.onclick = event => {
                        event.preventDefault();
                        this.files[event.target.dataset.index - 1] = null;
                        element.closest('span').remove();
                    });
                };
                this.uploadForm.querySelector('label').ondragover = event => {
                    event.preventDefault();
                    event.target.classList.add('dragover');
                };
                this.uploadForm.querySelector('label').ondragleave = event => {
                    event.preventDefault();
                    event.target.classList.remove('dragover');
                };
                this.uploadForm.querySelector('label').ondrop = event => {
                    event.preventDefault();
                    event.target.classList.remove('dragover');
                    for (let i = 0; i < event.dataTransfer.files.length; i++) {
                        let index = this.files.push(event.dataTransfer.files[i]);
                        this.uploadForm.querySelector('.files-list').innerHTML += '<span><i class="fa-solid fa-file"></i>' + event.dataTransfer.files[i].name + '<i class="fa-solid fa-xmark remove-file-btn" data-index="' + index + '"></i></span>';
                    }
                    this.uploadForm.querySelectorAll('.files-list .remove-file-btn').forEach(element => element.onclick = event => {
                        event.preventDefault();
                        this.files[event.target.dataset.index - 1] = null;
                        ...
                        req.onreadystatechange = () => {
                            if (req.readyState == 4 && req.status == 200) { 
                                try {
                                    ...
                                    this.uploadForm.querySelector('.msg').innerHTML = req.responseText;
                                }
                            }
                        };
                        req.send(formData);
