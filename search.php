...
echo '<div class="card">';
echo '<div class="card-header"><i class="fas fa-search"></i> Search</div>';
    ...
    if (strlen($word) < 2) {
        ...
        if ($count == 0) {
            ...
            foreach ($posts as $row) {
                ...
                echo '          <small>Posted by <b><i class="fas fa-user"></i> ' . htmlspecialchars(post_author($row['author_id'])) . '</b> on <b><i class="far fa-calendar-alt"></i> ' . date($settings['date_format'], strtotime($row['date'])) . '</b></small>';
                echo '          <small class="float-end"><i class="fas fa-comments"></i> <a href="post.php?name=' . htmlspecialchars($row['slug']) . '#comments" class="blog-comments"><b>' . post_commentscount($row['id']) . '</b></a></small>';
                echo '        </div>';
