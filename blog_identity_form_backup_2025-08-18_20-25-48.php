...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $data = [
            ...
            'blog_email' => sanitize_input($_POST['blog_email']),
...
<body>
    <div class="settings-form">
        <div class="nav-links">
            <a href="settings_dash.php">← Back to Settings Dashboard</a>
            <a href="blog_display_form.php">Blog Display →</a>
        </div>
        
        <h1>Blog Identity Settings</h1>
        <p>Configure your blog's basic identity information, author details, and meta data.</p>
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" id="blog_title" name="blog_title" 
                       value="<?= htmlspecialchars($current_settings['blog_title'] ?? '') ?>" required>
                <small>The main title of your blog that appears in headers and page titles</small>
            </div>
            
            <div class="form-group">
                <label for="blog_description">Blog Description</label>
                <textarea id="blog_description" name="blog_description"><?= htmlspecialchars($current_settings['blog_description'] ?? '') ?></textarea>
                <small>A brief description of your blog that appears in headers and meta tags</small>
            </div>
            
            <div class="form-group">
                <label for="blog_tagline">Blog Tagline</label>
                <input type="text" id="blog_tagline" name="blog_tagline" 
                       value="<?= htmlspecialchars($current_settings['blog_tagline'] ?? '') ?>">
                <small>A short, catchy phrase that summarizes your blog's purpose</small>
            </div>
            
            <div class="form-group">
                <label for="author_name">Default Author Name</label>
                <input type="text" id="author_name" name="author_name" 
                       value="<?= htmlspecialchars($current_settings['author_name'] ?? '') ?>">
                <small>The default author name for blog posts</small>
            </div>
            
            <div class="form-group">
                <label for="author_bio">Author Bio</label>
                <textarea id="author_bio" name="author_bio"><?= htmlspecialchars($current_settings['author_bio'] ?? '') ?></textarea>
                <small>A brief biography of the default blog author</small>
            </div>
            
            <div class="form-group">
                <label for="default_author_id">Default Author ID</label>
                <input type="number" id="default_author_id" name="default_author_id" 
                       value="<?= htmlspecialchars($current_settings['default_author_id'] ?? '1') ?>" min="1">
                <small>The user ID of the default blog author</small>
            </div>
            
            <div class="form-group">
                <label for="meta_description">Meta Description</label>
                <textarea id="meta_description" name="meta_description"><?= htmlspecialchars($current_settings['meta_description'] ?? '') ?></textarea>
                <small>SEO meta description for your blog (recommended: 150-160 characters)</small>
            </div>
            
            <div class="form-group">
                <label for="meta_keywords">Meta Keywords</label>
                <input type="text" id="meta_keywords" name="meta_keywords" 
                       value="<?= htmlspecialchars($current_settings['meta_keywords'] ?? '') ?>">
                <small>Comma-separated keywords that describe your blog content</small>
            </div>
            
            <div class="form-group">
                <label for="blog_email">Blog Email</label>
                <input type="email" id="blog_email" name="blog_email" 
                       value="<?= htmlspecialchars($current_settings['blog_email'] ?? '') ?>">
                <small>Contact email address for your blog</small>
            </div>
            
            <div class="form-group">
                <label for="blog_url">Blog URL</label>
                <input type="url" id="blog_url" name="blog_url" 
                       value="<?= htmlspecialchars($current_settings['blog_url'] ?? '') ?>">
                <small>The full URL where your blog can be accessed</small>
            </div>
            
            <div class="form-group">
                <label for="copyright_text">Copyright Text</label>
                <input type="text" id="copyright_text" name="copyright_text" 
                       value="<?= htmlspecialchars($current_settings['copyright_text'] ?? '') ?>">
                <small>Copyright notice that appears in your blog footer</small>
            </div>
