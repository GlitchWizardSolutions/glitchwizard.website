...
if (isset($_GET['action']) && $_GET['action'] == 'media') {
    ...
    if (isset($_POST['total_files']) && (int)$_POST['total_files'] > 0) {
        ...
        for ($i = 0; $i < (int)$_POST['total_files']; $i++) {
            ...
            if (isset($_FILES['file_' . $i]) && !empty($_FILES['file_' . $i]['tmp_name'])) {
                ...
                $stmt = $pdo->prepare('INSERT INTO shop_product_media (title, caption, date_uploaded, full_path) VALUES (?, ?, ?, ?)');
    ...
    if (isset($_GET['id'])) {
        ...
        $stmt = $pdo->prepare('SELECT * FROM shop_product_media WHERE id = ?');
    ...
    if (isset($_GET['id'], $_POST['title'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shop_product_media SET title = ?, caption = ?, date_uploaded = ?, full_path = ? WHERE id = ?');
    ...
    if (isset($_GET['id'], $_GET['delete'])) {
        ...
        $stmt = $pdo->prepare('DELETE m, pm FROM shop_product_media m LEFT JOIN shop_product_media_map pm ON pm.media_id = m.id WHERE m.id = ?');
        $stmt->execute([ $_GET['id'] ]);
    }
    // Get all media from database
    $stmt = $pdo->prepare('SELECT *, DATE_FORMAT(date_uploaded, "%Y-%m-%d %H:%i") AS date_uploaded FROM shop_product_media ORDER BY date_uploaded DESC');
    $stmt->execute();
...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] == 'get_customer_details') {
    ...
    $stmt = $pdo->prepare('SELECT a.*, (SELECT COUNT(*) FROM shop_transactions t WHERE t.account_id = a.id) AS total_orders FROM accounts a WHERE a.id = ?');
