...
$stmt = $pdo->prepare('SELECT * FROM shop_products ORDER BY id');
$stmt->execute();
    ...
    if (isset($_POST['item_id']) && is_array($_POST['item_id']) && count($_POST['item_id']) > 0) {
        ...
        for ($i = 0; $i < count($_POST['item_id']); $i++) {
            ...
            if (!intval($_POST['item_id'][$i])) {
                ...
                $stmt = $pdo->prepare('UPDATE shop_transaction_items SET txn_id = ?, item_id = ?, item_price = ?, item_quantity = ?, item_options = ? WHERE id = ?');
        ...
        $stmt = $pdo->prepare('DELETE FROM shop_transaction_items WHERE txn_id = ? AND id NOT IN (' . $in . ')');
        $stmt->execute(array_merge([ $txn_id ], $delete_list));
    } else {
        // No item exists, delete all
        $stmt = $pdo->prepare('DELETE FROM shop_transaction_items WHERE txn_id = ?');
        $stmt->execute([ $txn_id ]);       
    }
}
// Save captured data
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM shop_transactions WHERE id = ?');
    $stmt->execute([ $_GET['id'] ]);
    $transaction = $stmt->fetch(PDO::FETCH_ASSOC);
    // Retrieve the transaction items from the database
    $stmt = $pdo->prepare('SELECT * FROM shop_transaction_items WHERE txn_id = ?');
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shop_transactions SET txn_id = ?, payment_amount = ?, payment_status = ?, created = ?, payer_email = ?, first_name = ?, last_name = ?, address_street = ?, address_city = ?, address_state = ?, address_zip = ?, address_country = ?, account_id = ?, payment_method = ?, discount_code = ?, shipping_method = ?, shipping_amount = ?, tax_amount = ? WHERE id = ?');
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO shop_transactions (txn_id,payment_amount,payment_status,created,payer_email,first_name,last_name,address_street,address_city,address_state,address_zip,address_country,account_id,payment_method,discount_code,shipping_method,shipping_amount,tax_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
