...
$stmt = $pdo->query('SELECT id, title FROM shop_product_categories');
$stmt->execute();
$categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
// Get all the products from the database
$stmt = $pdo->query('SELECT id, title FROM shop_products');
$stmt->execute();
...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shop_discounts SET category_ids = ?, product_ids = ?, discount_code = ?, discount_type = ?, discount_value = ?, start_date = ?, end_date = ? WHERE id = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM shop_discounts WHERE id = ?');
    $stmt->execute([ $_GET['id'] ]);
    $discount = $stmt->fetch(PDO::FETCH_ASSOC);
    // Get the discount categories
    $stmt = $pdo->prepare('SELECT c.title, c.id FROM shop_discounts d JOIN shop_product_categories c ON FIND_IN_SET(c.id, d.category_ids) WHERE d.id = ?');
    $stmt->execute([ $_GET['id'] ]);
    $discount['categories'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the discount products
    $stmt = $pdo->prepare('SELECT p.title, p.id FROM shop_discounts d JOIN shop_products p ON FIND_IN_SET(p.id, d.product_ids) WHERE d.id = ?');
    $stmt->execute([ $_GET['id'] ]);
    ...
    if (isset($_POST['submit'])) {
        $categories_list = isset($_POST['categories']) ? implode(',', $_POST['categories']) : '';
        $products_list = isset($_POST['products']) ? implode(',', $_POST['products']) : '';
        $stmt = $pdo->prepare('INSERT INTO shop_discounts (category_ids,product_ids,discount_code,discount_type,discount_value,start_date,end_date) VALUES (?,?,?,?,?,?,?)');
