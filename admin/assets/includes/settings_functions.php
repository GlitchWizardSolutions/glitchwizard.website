...
function format_var_html($key, $value, $comment, $list = []) {
    ...
    if ($comment && substr($comment, 0, 2) === '//') {
        ...
        $html .= ' <i class="fas fa-info-circle text-muted" data-bs-toggle="tooltip" title="' . $commentText . '"></i>';
    }
...
function format_tabs($contents) {
    ...
    $tabs = [
        ['name' => 'Environment', 'id' => 'tab-env', 'icon' => 'fas fa-server'],
        ['name' => 'Database', 'id' => 'tab-db', 'icon' => 'fas fa-database'],
        ['name' => 'URLs & Paths', 'id' => 'tab-urls', 'icon' => 'fas fa-link'],
        ['name' => 'Security', 'id' => 'tab-security', 'icon' => 'fas fa-shield-alt']
    ];
