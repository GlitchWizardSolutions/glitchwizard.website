...
<div class="content-title mb-4" id="main-tickets-list" role="banner" aria-label="Tickets Management Header">
    <div class="title">
        <div class="icon">
            <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 576 512"><!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                <path
                    d="M64 64C28.7 64 0 92.7 0 128v64c0 8.8 7.4 15.7 15.7 18.6C34.5 217.1 48 235 48 256s-13.5 38.9-32.3 45.4C7.4 304.3 0 311.2 0 320v64c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V320c0-8.8-7.4-15.7-15.7-18.6C541.5 294.9 528 277 528 256s13.5-38.9 32.3-45.4c8.3-2.9 15.7-9.8 15.7-18.6V128c0-35.3-28.7-64-64-64H64zm368 96a32 32 0 1 1 0 64 32 32 0 1 1 0-64zM96 192a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zM464 320a32 32 0 1 1 0 64 32 32 0 1 1 0-64zM96 352a32 32 0 1 1 64 0 32 32 0 1 1 -64 0z" />
            </svg>
        </div>
        <div class="txt">
            <h2>Tickets</h2>
    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <small class="text-muted">
                Reply Legend: 
                <i class="fas fa-user-tie text-success" title="Admin responded last"></i> Admin |
                <i class="fas fa-user text-warning" title="Waiting for admin response"></i> Customer |
                <i class="fas fa-circle text-muted" title="No activity"></i> None
            </small>
        </div>

        <div class="table-responsive">
            <table class="table table-hover mb-0" role="grid" aria-label="Tickets">
                <thead class="table-light" role="rowgroup">
                    <tr role="row">
                        <th class="text-start" role="columnheader" scope="col" style="width: 80px; padding-right: 20px;"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=id'?>" class="text-decoration-none">#<?php if ($order_by=='id'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-start" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=full_name'?>" class="text-decoration-none">User<?php if ($order_by=='full_name'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-start" role="columnheader" scope="col" style="padding-right: 10px;"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=title'?>" class="text-decoration-none">Title<?php if ($order_by=='title'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-start" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=ticket_status'?>" class="text-decoration-none">Status<?php if ($order_by=='ticket_status'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-center d-none d-md-table-cell" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=reply'?>" class="text-decoration-none">Reply<?php if ($order_by=='reply'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-center d-none d-lg-table-cell" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=priority'?>" class="text-decoration-none">Priority<?php if ($order_by=='priority'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-center d-none d-lg-table-cell" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=category'?>" class="text-decoration-none">Category<?php if ($order_by=='category'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-center" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=approved'?>" class="text-decoration-none">Approved<?php if ($order_by=='approved'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                        <th class="text-center d-none d-md-table-cell" role="columnheader" scope="col"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=created'?>" class="text-decoration-none">Date<?php if ($order_by=='created'): ?><span aria-label="Sorted <?= $order == 'ASC' ? 'ascending' : 'descending' ?>"><?= $order == 'ASC' ? '▲' : '▼' ?></span><?php endif; ?></a></th>
                ...
                <tr role="row">
                    ...
                    <td class="text-center d-none d-md-table-cell">
                        ...
                        if (!isset($ticket['reply']) || $ticket['reply'] == 1) {
                            echo '<i class="fas fa-circle text-muted" title="No activity"></i>';
                        } elseif ($ticket['reply'] == $_SESSION['account_id']) {
                            echo '<i class="fas fa-user-tie text-success" title="Admin responded last"></i>';
                        } else {
                            echo '<i class="fas fa-user text-warning" title="Waiting for admin response"></i>';
                        }
                        ?>
                    </td>
                    ...
                    <td class="text-center"><?=$ticket['approved'] ? '<i class="fas fa-check text-success"></i>' : '<i class="fas fa-times text-danger"></i>'?></td>
        ...
        <nav aria-label="Tickets pagination">
            <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted">
                    Showing page <?=$pagination_page?> of <?=ceil($tickets_total / $results_per_page) == 0 ? 1 : ceil($tickets_total / $results_per_page)?> 
