<?php
/* Content Settings - Professional Content Management Interface
 * Manages Hero Section, Services, About Pages, and other site content
 * Features: Image uploads, rich text editing, organized tabs
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($pdo) {
    try {
      $existingHero = $pdo->query("SELECT hero_headline, hero_subheadline, hero_background_image, hero_button_text, hero_button_link, hero_form_top_text, hero_form_side_text, hero_form_button_text FROM setting_content_homepage LIMIT 1")->fetch(PDO::FETCH_ASSOC) ?: [];
  ...
  $hero_form_top_text_p = $is_upload_only ? ($existingHero['hero_form_top_text'] ?? null) : (array_key_exists('hero_form_top_text', $posted_hero) ? trim($posted_hero['hero_form_top_text']) : ($existingHero['hero_form_top_text'] ?? null));
  $hero_form_side_text_p = $is_upload_only ? ($existingHero['hero_form_side_text'] ?? null) : (array_key_exists('hero_form_side_text', $posted_hero) ? trim($posted_hero['hero_form_side_text']) : ($existingHero['hero_form_side_text'] ?? null));
  $hero_form_button_text_p = $is_upload_only ? ($existingHero['hero_form_button_text'] ?? null) : (array_key_exists('hero_form_button_text', $posted_hero) ? trim($posted_hero['hero_form_button_text']) : ($existingHero['hero_form_button_text'] ?? null));
  ...
  if (!empty($_FILES['home_hero_image']) && $_FILES['home_hero_image']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if($pdo && $success!==false) {
      try {
        $stmt = $pdo->prepare("UPDATE setting_content_homepage SET hero_headline=?, hero_subheadline=?, hero_background_image=?, hero_button_text=?, hero_form_top_text=?, hero_form_side_text=?, hero_form_button_text=? LIMIT 1");
        $stmt->execute([$hero_headline_p,$hero_subheadline_p,$hero_image_select_p,$hero_button_text_p,$hero_form_top_text_p,$hero_form_side_text_p,$hero_form_button_text_p]);
...
$hero_form_top_text=$hero_form_side_text=$hero_form_button_text='';
if($pdo){ $stmt=$pdo->query("SELECT hero_headline, hero_subheadline, hero_background_image, hero_button_text, hero_button_link, hero_form_top_text, hero_form_side_text, hero_form_button_text FROM setting_content_homepage LIMIT 1"); if($row=$stmt->fetch(PDO::FETCH_ASSOC)){ $hero_headline=$row['hero_headline'];$hero_subheadline=$row['hero_subheadline'];$hero_image=$row['hero_background_image'];$hero_button_text=$row['hero_button_text'];$hero_button_link=$row['hero_button_link'];$hero_form_top_text=$row['hero_form_top_text']??'';$hero_form_side_text=$row['hero_form_side_text']??'';$hero_form_button_text=$row['hero_form_button_text']??''; } }
  ...
  <div class="card-header py-4 px-4 branding-settings-header">
    <div class="d-flex justify-content-between align-items-center">
      <h6 class="m-0 font-weight-bold">
        <span class="header-icon"><i class="bi bi-pencil-square" aria-hidden="true"></i></span>
        Content Settings
      </h6>
      <span class="text-white" style="font-size: 0.875rem;">Manage website content & media</span>
    </div>
  </div>
  <div class="card-body">
      ...
      <form action="" method="post" enctype="multipart/form-data" id="heroSettingsForm" class="mt-2">
        ...
        <div class="row">
          <div class="col-md-12 mb-3">
            <label class="form-label fw-bold" for="home_hero_form_top_text">Hero Form Top Text</label>
            <input type="text" name="home[hero_form_top_text]" id="home_hero_form_top_text" class="form-control" value="<?= htmlspecialchars($hero_form_top_text) ?>" placeholder="Text that appears above the form">
            <div class="form-text text-muted">Example: "No obligation. No spam. Just a real offer from your local neighbor."</div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-4 mb-3">
            <label class="form-label fw-bold" for="home_hero_form_side_text">Hero Form Side Text</label>
            <input type="text" name="home[hero_form_side_text]" id="home_hero_form_side_text" class="form-control" value="<?= htmlspecialchars($hero_form_side_text) ?>" placeholder="Text on the left of the form">
            <div class="form-text text-muted">Currently: "GET STARTED!"</div>
          </div>
          <div class="col-md-4 mb-3">
            <label class="form-label fw-bold" for="home_hero_form_button_text">Hero Form Button Text</label>
            <input type="text" name="home[hero_form_button_text]" id="home_hero_form_button_text" class="form-control" value="<?= htmlspecialchars($hero_form_button_text) ?>" placeholder="Button text">
            <div class="form-text text-muted">Currently: "Get My Offer"</div>
          </div>
          <div class="col-md-4 mb-3">
            <label class="form-label fw-bold" for="home_hero_button_link">Hero CTA Button Link (coming soon)</label>
        ...
        <div class="hero-section-footer">
          <i class="bi bi-info-circle me-2"></i>Hero section changes are saved automatically and will appear on your homepage immediately.
        </div>
      </form>
...
<style>
/* Branding Card Header Styling (copied from branding_settings.php) */
...
.hero-section-divider {
  border-top: 2px solid var(--brand-primary-color, #3b89b0);
  margin: 30px 0;
  opacity: 0.8;
}
...
#hero-tab .form-control:focus,
#hero-tab .form-select:focus,
#hero-tab textarea:focus {
  background-color: #000 !important;
  color: #fff !important;
  border-color: var(--brand-primary-color, #3b89b0) !important;
  box-shadow: 0 0 0 0.2rem rgba(59, 137, 176, 0.25) !important;
}
...
/* Hero Image Selection Border */
#hero-tab .hero-thumb.selected {
  border-color: var(--brand-primary-color, #3b89b0) !important;
  box-shadow: 0 0 0 2px rgba(59, 137, 176, 0.35) !important;
}
...
.tab-btn {
  background: var(--brand-primary-color, #3b89b0) !important;
  border: 2px solid var(--brand-primary-color, #3b89b0) !important;
