...
function format_var_html($key, $value, $comment, $list = []) {
    ...
    if (substr($comment, 0, 2) === '//') {
        $html .= '<div class="form-text mb-2">' . ltrim($comment, '//') . '</div>';
    }
    if ($type == 'checkbox') {
        $html .= '<input type="hidden" name="' . $key . '" value="false">';
        $html .= '<div class="form-check">';
        $html .= '<input type="checkbox" name="' . $key . '" id="' . $key . '" class="form-check-input" value="true"' . $checked . '>';
        $html .= '<label class="form-check-label" for="' . $key . '">' . format_key($key) . '</label>';
        $html .= '</div>';
    } else if ($list) {
        ...
        $html .= '<textarea name="' . $key . '" id="' . $key . '" class="form-control" placeholder="' . format_key($key) . '">' . str_replace('\n', PHP_EOL, $value) . '</textarea>';
    } else {
        $html .= '<input type="' . $type . '" name="' . $key . '" id="' . $key . '" class="form-control" value="' . $value . '" placeholder="' . format_key($key) . '">';
    }
    $html .= '</div>';
    return $html;
}
...
?>
<?=template_admin_header('Invoice Settings', 'invoices', 'settings')?>

<form action="" method="post">
    <div class="content-header">
        <div class="content-title responsive-flex-wrap responsive-pad-bot-3">
            <h2 class="responsive-width-100">Invoice System Settings</h2>
        ...
        <p class="text-muted">Configure your invoice system settings and preferences.</p>
    </div>
