...
if (isset($_FILES['collections_file'], $_POST['import_collections']) && !empty($_FILES['collections_file']['tmp_name'])) {
    ...
    if (isset($data) && !empty($data)) {    
        ...
        foreach ($data as $k => $row) {
            ...
            // insert into database
            // tip: if you want to update existing records, use INSERT ... ON DUPLICATE KEY UPDATE instead
            $stmt = $pdo->prepare('INSERT IGNORE INTO invoices VALUES (' . $values . ')');
            $stmt->execute(array_values($row));
            $i += $stmt->rowCount();
        }
        header('Location: invoices.php?success_msg=4&imported=' . $i);
...
if (isset($_POST['export_clients'], $_POST['polls_export_type'])) {
    ...
    if ($_POST['polls_export_type'] == 'xml') {
        $filename = 'invoice_clients.xml';
        $fp = fopen('php://output', 'w');
        header('Content-type: application/xml');
        header('Content-Disposition: attachment; filename=' . $filename);
        fwrite($fp, '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL);
        fwrite($fp, '<invoice_clients>' . PHP_EOL);
        foreach ($polls as $poll) {
            fwrite($fp, '    <client>' . PHP_EOL);
            foreach ($poll as $key => $value) {
...
// Handle Invoice Export form submission
if (isset($_POST['export_collections'], $_POST['categories_export_type'])) {
    // Get all invoices
    $result = $pdo->query('SELECT * FROM invoices ORDER BY id ASC');
    $poll_categories = [];
    $columns = [];
    // Fetch all records into an associative array
    if ($result->rowCount() > 0) {
    ...
    if ($_POST['categories_export_type'] == 'xml') {
        ...
        header('Content-Disposition: attachment; filename=' . $filename);
        fwrite($fp, '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL);
        fwrite($fp, '<invoices>' . PHP_EOL);
        foreach ($poll_categories as $poll_category) {
            fwrite($fp, '    <invoice>' . PHP_EOL);
            foreach ($poll_category as $key => $value) {
                fwrite($fp, '        <' . $key . '>' . htmlspecialchars($value) . '</' . $key . '>' . PHP_EOL);
            }
            fwrite($fp, '    </invoice>' . PHP_EOL);
...
<?=template_admin_header('Import/Export Invoice Data', 'invoices', 'transfer')?>
...
<div class="content-title mb-4">
    <div class="title">
        ...
        <div class="txt">
            ...
            <p>Import client and invoice data from files or export existing data to various formats.</p>
        </div>
    </div>
</div>
...
<form action="" method="post" enctype="multipart/form-data" role="form">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <p class="text-muted mb-0 mt-1">Import or export client and invoice data using the tabs below</p>
        </div>
        <div class="card-body">
            <!-- Tab Navigation -->
