<?php
/* 
 * Polls Table Transfer System
 * 
 * SYSTEM: GWS Universal Hybrid App - Admin Panel
 * FILE: polls_table_transfer.php
 * LOCATION: /public_html/admin/polling_system/
 * PURPOSE: Import and export poll and poll category data in multiple formats (CSV, JSON, XML, TXT)
 * 
 * FILE RELATIONSHIP:
 ...
 * Tabbed user interface for polls and poll categories
 * Bootstrap 5 styling
 */
...
// Handle Polls Import form submission
if (isset($_FILES['polls_file'], $_POST['import_polls']) && !empty($_FILES['polls_file']['tmp_name'])) {
    // check type
    $type = pathinfo($_FILES['polls_file']['name'], PATHINFO_EXTENSION);
    $data = [];
    if ($type == 'csv') {
        $file = fopen($_FILES['polls_file']['tmp_name'], 'r');
        $header = fgetcsv($file);
        ...
        $data = json_decode(file_get_contents($_FILES['polls_file']['tmp_name']), true);
    } elseif ($type == 'xml') {
        $xml = simplexml_load_file($_FILES['polls_file']['tmp_name']);
    ...
    if (isset($data) && !empty($data)) {    
        ...
        foreach ($data as $k => $row) {
            ...
            $stmt = $pdo->prepare('INSERT IGNORE INTO polls VALUES (' . $values . ')');
            $stmt->execute(array_values($row));
            $i += $stmt->rowCount();
        }
        header('Location: polls.php?success_msg=4&imported=' . $i);
        exit;
    }
}

// Handle Poll Categories Import form submission
if (isset($_FILES['categories_file'], $_POST['import_categories']) && !empty($_FILES['categories_file']['tmp_name'])) {
    // check type
    $type = pathinfo($_FILES['categories_file']['name'], PATHINFO_EXTENSION);
    $data = [];
    if ($type == 'csv') {
        $file = fopen($_FILES['categories_file']['tmp_name'], 'r');
        $header = fgetcsv($file);
        while ($row = fgetcsv($file)) {
            $data[] = array_combine($header, $row);
        }
        ...
        $data = json_decode(file_get_contents($_FILES['categories_file']['tmp_name']), true);
    } elseif ($type == 'xml') {
        $xml = simplexml_load_file($_FILES['categories_file']['tmp_name']);
        $data = json_decode(json_encode($xml), true)['item'];
    } elseif ($type == 'txt') {
        $file = fopen($_FILES['categories_file']['tmp_name'], 'r');
        while ($row = fgetcsv($file)) {
    ...
    if (isset($data) && !empty($data)) {    
        ...
        foreach ($data as $k => $row) {
            ...
            // insert into database
            // tip: if you want to update existing records, use INSERT ... ON DUPLICATE KEY UPDATE instead
            $stmt = $pdo->prepare('INSERT IGNORE INTO polls_categories VALUES (' . $values . ')');
            $stmt->execute(array_values($row));
            $i += $stmt->rowCount();
        }
        header('Location: poll_categories.php?success_msg=4&imported=' . $i);
        exit;
    }
}

// Handle Polls Export form submission
if (isset($_POST['export_polls'], $_POST['polls_export_type'])) {
    // Get all polls
    $result = $pdo->query('SELECT * FROM polls ORDER BY id ASC');
    $polls = [];
    $columns = [];
...
if (isset($_POST['export_categories'], $_POST['categories_export_type'])) {
    // Get all poll_categories
    $result = $pdo->query('SELECT * FROM polls_categories ORDER BY id ASC');
    $poll_categories = [];
    $columns = [];
    // Fetch all records into an associative array
    if ($result->rowCount() > 0) {
...
<?=template_admin_header('Import/Export Polls', 'polls', 'bulk')?>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="pt-3 border-top mt-4">
                    <button type="submit" name="export_polls" class="btn btn-success">
    ...
    <div id="import-polls-tab" 
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <!-- Bottom action buttons for this tab -->
                <div class="pt-3 border-top mt-4">
                    <button type="submit" name="import_polls" class="btn btn-success">
    ...
    <div id="export-categories-tab" 
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <!-- Bottom action buttons for this tab -->
                <div class="pt-3 border-top mt-4">
                    <button type="submit" name="export_categories" class="btn btn-success">
                        <i class="bi bi-download me-1" aria-hidden="true"></i>
                        Export Categories
                    </button>
                </div>
    ...
    <div id="import-categories-tab" 
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <!-- Bottom action buttons for this tab -->
                <div class="pt-3 border-top mt-4">
                    <button type="submit" name="import_categories" class="btn btn-success">
                        <i class="bi bi-upload me-1" aria-hidden="true"></i>
                        Import Categories
                    </button>
                </div>
